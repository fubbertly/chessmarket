<section>
    ...
    <form id="profile-update-form" method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div class="flex items-center gap-4">
            <x-primary-button id="save-button" type="submit" disabled>
                {{ __('Save') }}
            </x-primary-button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        emailInput.addEventListener('input', function() {
            if (emailInput.value.trim() !== originalEmail) {
                saveButton.classList.remove('cursor-not-allowed', 'bg-gray-300', 'hover:bg-gray-400');
                saveButton.classList.add('bg-blue-500', 'hover:bg-blue-600');
                saveButton.disabled = false;
            } else {
                saveButton.classList.remove('bg-blue-500', 'hover:bg-blue-600');
                saveButton.classList.add('cursor-not-allowed', 'bg-gray-300', 'hover:bg-gray-400');
